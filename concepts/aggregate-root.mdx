...
description: "AggregateRoot is the base class for domain entities in ts-mongo-criteria. Implement getId(), toPrimitives(), and fromPrimitives() to integrate with MongoRepository."
---
...
| `delete(filter, options?)` | Delete documents matching a raw MongoDB filter. Pass `{ deleteOne: true }` in options for a single-document delete. |
